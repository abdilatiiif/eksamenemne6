...
function KlimaCalcpage() {
  useEffect(() => {
    const lagret = localStorage.getItem("lagret");
    if (lagret) {
      try {
        setlagretResultat(JSON.parse(lagret));
      } catch (error) {
        console.error("Kunne ikke laste lagrede resultater:", error);
      }
    }
  }, []);
  ...
  // lagring av resultat i localStorage
  const [lagretResultat, setlagretResultat] = useState([]);
  ...
  const kalkulerKlimafotavtrykk = useCallback(() => {
    ...
    // Mat (per år)
    const kjøttUtslipp =
      ((formdata.beef + formdata.kjøttretter) * 52 * 6) / 1000; // 6 kg CO₂ per kg kjøtt (gjennomsnitt for ulike typer kjøtt).
  ...
  const lagreResultat = () => {
    ...
    const newResult = {
      ...
      mat: results.mat,
      energi: results.energi,
    };

    const updatedResults = [newResult, ...lagretResultat].slice(0, 5); // Behold kun de 5 siste resultatene

    setlagretResultat(updatedResults);
    localStorage.setItem("lagret", JSON.stringify(updatedResults)); // Lagre i localStorage
  };

  // Slett alle lagrede resultater
  const slettLagret = () => {
    setlagretResultat([]);
    localStorage.removeItem("lagret");
    alert("Alle lagrede resultater er slettet");
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          ...
          <div>
            ...
            {results && (
              <div
                className={`${
                  dropdown && "slide-out-bottom"
                } bg-green-600 text-white p-6 rounded-lg shadow-lg sticky top-4`}
              >
            ...
            {lagretResultat.length > 0 && (
              <div className="mt-6 bg-white p-6 rounded-lg shadow-lg">
                ...
                <div className="space-y-3">
                  {lagretResultat.map((result, index) => (
                    <div
                      key={index}
                    ...
                    >
                      ...
                      <div className="font-bold text-green-800 text-lg">
                        {result.total} tonn CO₂/år
                      </div>
                      <div className="text-sm text-gray-600 mt-1">
                        Transport: {result.transport} | Mat: {result.mat} |
